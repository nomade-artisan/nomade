...
import { Search, Plus, Upload, X, Trash2, Pencil } from "lucide-react";
...
export default function ProductsView({ products, fetchProducts }: Props) {
  ...
  const handleCreateProduct = async () => {
    ...
    const { error: uploadError } = await supabase.storage.from("products").upload(fileName, newProduct.image);
  ...
  return (
    <div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="min-w-0">
                    <p className="text-sm md:text-[15px] font-light truncate text-stone-900">{product.name}</p>
                    <p className="text-[11px] md:text-xs text-stone-400 mt-0.5">{product.category} — {product.price}€</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="flex items-center gap-1.5 md:gap-2">
                    ...
                    >−</button>
                    <span className={`text-[10px] md:text-xs px-2 md:px-3 py-0.5 md:py-1 rounded-full ${stockBadge(product.stock)}`}>
      ...
      {/* ==================== MODAL CRÉATION ==================== */}
      {showCreateModal && (
        <div className="fixed inset-0 z-[100] bg-black/40 flex items-end md:items-center justify-center" onClick={() => setShowCreateModal(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full md:max-w-xl bg-white rounded-t-[24px] md:rounded-[32px] p-4 md:p-6 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4 md:mb-5">
              <h2 className="text-lg md:text-2xl font-light">Nouveau produit</h2>
              <button onClick={() => setShowCreateModal(false)} className="p-2 -mr-2"><X size={18} /></button>
            </div>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <select value={newProduct.category}
                  ...
                  <option>Cuir</option><option>Minimal</option><option>Bandoulière</option><option>Aventure</option><option>Accessoires</option>
                </select>
              </div>
              ...
              <textarea placeholder="Détails (un par ligne) *" value={newProduct.details}
                onChange={(e) => setNewProduct({ ...newProduct, details: e.target.value })}
                className="w-full min-h-[70px] md:min-h-[80px] rounded-xl border px-4 py-3 text-sm resize-none" />
              <label className="block border-2 border-dashed rounded-2xl p-5 md:p-6 text-center cursor-pointer">
                {newProduct.imagePreview ? (
                  <img src={newProduct.imagePreview} className="h-28 md:h-32 mx-auto rounded-xl object-cover" />
                ) : (
                  <div><Upload className="mx-auto text-stone-300 mb-2" size={24} /><p className="text-xs text-stone-400">Ajouter une image</p></div>
                )}
                <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file) setNewProduct({ ...newProduct, image: file, imagePreview: URL.createObjectURL(file) });
                }} />
              </label>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowCreateModal(false)}
                  className="flex-1 h-10 md:h-11 rounded-full border border-stone-200 text-stone-600 text-xs uppercase tracking-wider">Annuler</button>
                <button onClick={handleCreateProduct}
                  className="flex-1 h-10 md:h-11 rounded-full bg-stone-900 text-white text-xs uppercase tracking-wider">Créer</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showEditModal && editingProduct && (
        <div className="fixed inset-0 z-[100] bg-black/40 flex items-end md:items-center justify-center" onClick={() => setShowEditModal(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full md:max-w-xl bg-white rounded-t-[24px] md:rounded-[32px] p-4 md:p-6 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4 md:mb-5">
              <h2 className="text-lg md:text-2xl font-light">Modifier le produit</h2>
              <button onClick={() => setShowEditModal(false)} className="p-2 -mr-2"><X size={18} /></button>
            </div>
            <div className="space-y-3">
              ...
              <textarea value={editingProduct.description}
                ...
                className="w-full min-h-[70px] md:min-h-[80px] rounded-xl border px-4 py-3 text-sm resize-none" />
              <textarea value={editingProduct.details.join("\n")}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {editingProduct.images.map((img, i) => (
                      <div key={i} className="relative w-12 h-12 md:w-14 md:h-14 rounded-lg overflow-hidden">
                        <img src={img} className="w-full h-full object-cover" />
                        <button onClick={() => removeImage(editingProduct.id, img)}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleEditProduct}
                  className="flex-1 h-10 md:h-11 rounded-full bg-stone-900 text-white text-xs uppercase tracking-wider">Sauvegarder</button>
              </div>
            </div>
          </div>
        </div>
